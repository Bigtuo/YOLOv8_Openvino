...
class YOLOv7_pose:
    """YOLOv7_pose detection model class for handling inference and visualization."""
    ...
    def postprocess(self, preds, im0, ratio, pad_w, pad_h, conf_threshold, iou_threshold):
        ...
        Returns:
            boxes (List): list of bounding boxes.
        """
        x = preds  # outputs: predictions (1, 8400*3, 85)，其中56=4+1+17*3，17个关键点(x,y,visibility)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='weights/yolov7-w6-pose.onnx', help='Path to ONNX model')
    ...
    # Build model
    model = YOLOv7_pose(args.model, args.imgsz, args.infer_tool)
